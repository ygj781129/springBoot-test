 ...
 * @com.example.limitierredis.config {Object} vars 创建imageUploader时所需要的参数
 * @com.example.limitierredis.config {Number} vars.gridWidth 每一个预览图片所占的宽度，应该为flash寛的整除
 * @com.example.limitierredis.config {Number} vars.gridHeight 每一个预览图片所占的高度，应该为flash高的整除
 * @com.example.limitierredis.config {Number} vars.picWidth 单张预览图片的宽度
 * @com.example.limitierredis.config {Number} vars.picHeight 单张预览图片的高度
 * @com.example.limitierredis.config {String} vars.uploadDataFieldName POST请求中图片数据的key,默认值'picdata'
 * @com.example.limitierredis.config {String} vars.picDescFieldName POST请求中图片描述的key,默认值'picDesc'
 * @com.example.limitierredis.config {Number} vars.maxSize 文件的最大体积,单位'MB'
 * @com.example.limitierredis.config {Number} vars.compressSize 上传前如果图片体积超过该值，会先压缩
 * @com.example.limitierredis.config {Number} vars.maxNum:32 最大上传多少个文件
 * @com.example.limitierredis.config {Number} vars.compressLength 能接受的最大边长，超过该值会等比压缩
 * @com.example.limitierredis.config {String} vars.url 上传的url地址
 * @com.example.limitierredis.config {Number} vars.mode mode == 0时，是使用滚动条，mode == 1时，拉伸flash, 默认值为0
 ...
 * @com.example.limitierredis.config {Object} createOptions 创建flash时需要的参数，请参照baidu.swf.create文档
 * @com.example.limitierredis.config {String} createOptions.width
 * @com.example.limitierredis.config {String} createOptions.height
 * @com.example.limitierredis.config {Number} maxNum 最大可选文件数
 * @com.example.limitierredis.config {Function|String} selectFile
 * @com.example.limitierredis.config {Function|String} exceedMaxSize
 * @com.example.limitierredis.config {Function|String} deleteFile
 * @com.example.limitierredis.config {Function|String} uploadStart
 * @com.example.limitierredis.config {Function|String} uploadComplete
 * @com.example.limitierredis.config {Function|String} uploadError
 * @com.example.limitierredis.config {Function|String} uploadProgress
 */
 ...
 * @com.example.limitierredis.config {String} [charset] script的字符集
 * @com.example.limitierredis.config {Integer} [timeOut] 超时时间，超过这个时间将不再响应本请求，并触发onfailure函数
 * @com.example.limitierredis.config {Function} [onfailure] timeOut设定后才生效，到达超时时间时触发本函数
 * @remark
 ...
 * @com.example.limitierredis.config {string} [charset] script的字符集
 * @com.example.limitierredis.config {string} [queryField] 服务器端callback请求字段名，默认为callback
 * @com.example.limitierredis.config {Integer} [timeOut] 超时时间(单位：ms)，超过这个时间将不再响应本请求，并触发onfailure函数
 * @com.example.limitierredis.config {Function} [onfailure] timeOut设定后才生效，到达超时时间时触发本函数
 * @remark
